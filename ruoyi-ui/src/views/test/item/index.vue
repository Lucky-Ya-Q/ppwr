<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="是否回收" prop="isRecycle">
        <el-select v-model="queryParams.isRecycle" placeholder="请选择是否回收" clearable size="small">
          <el-option
            v-for="dict in dict.type.pw_is_recycle"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
      ...
      <el-form-item label="是否退货" prop="isReturn">
        <el-select v-model="queryParams.isReturn" placeholder="请选择是否退货" clearable size="small">
          <el-option
            v-for="dict in dict.type.pw_is_return"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="itemList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="是否回收" align="center" prop="isRecycle">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.pw_is_recycle" :value="scope.row.isRecycle"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="是否退货" align="center" prop="isReturn">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.pw_is_return" :value="scope.row.isReturn"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="农药名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入农药名称" />
        </el-form-item>
        <el-form-item label="产品包装">
          <imageUpload v-model="form.imgs"/>
        </el-form-item>
        <el-form-item label="生产厂家" prop="manufacturer">
          <el-input v-model="form.manufacturer" placeholder="请输入生产厂家" />
        </el-form-item>
        <el-form-item label="是否回收" prop="isRecycle">
          <el-select v-model="form.isRecycle" placeholder="请选择是否回收">
            <el-option
              v-for="dict in dict.type.pw_is_recycle"
              :key="dict.value"
              :label="dict.label"
...
<script>
import { listItem, getItem, delItem, addItem, updateItem } from "@/api/test/item";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除订单项编号为"' + ids + '"的数据项？').then(function() {
        return delItem(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('test/item/export', {
        ...this.queryParams
